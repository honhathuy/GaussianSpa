...
from gaussian_renderer import render, network_gui, render_dash
import sys
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        render_pkg = render_dash(viewpoint_cam, gaussians, pipe, bg, use_trained_exp=dataset.train_test_exp, separate_sh=SPARSE_ADAM_AVAILABLE, render_size=gt_image.shape[-2:])
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, opt,iteration, Ll1, loss, l1_loss, Llag, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background))
            if (iteration in saving_iterations):
                scene.save(iteration)

            # DashGaussian Densification
            if iteration < opt.optimizing_spa_start_iter:
                ...
                optimizingSpa.update(imp_score=None)
            # Optimizer step
            if iteration < opt.iterations:
            ...
            if (iteration == opt.optimizing_spa_stop_iter):
                if  opt.optimizing_spa == True:
                    opacity = gaussians.get_opacity
                    k_threshold = int((1-opt.prune_ratio2) * opacity.shape[0])
                    _, indices = torch.topk(opacity[:, 0], k=k_threshold, largest=True)
                    ...
                    print("\nafter pruning",len(gaussians.get_opacity))
...
def prepare_output_and_logger(args):    
    ...
    if TENSORBOARD_FOUND:
        ...
        print("Tensorboard not available: not ging progress")
    return tb_writer
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[7000,op.iterations])
